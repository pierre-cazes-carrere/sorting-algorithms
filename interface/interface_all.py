...
class SortingAppWithSeaborn:
    ...
    def _setup_ui(self):
        ...
        self.selected_algo = ctk.StringVar(value="Tri par tas")
        self.algo_choice = ctk.CTkComboBox(self.root, values=list(self.algorithms.keys()), variable=self.selected_algo)
    ...
    def _run_sorting(self):
            ...
            algo_func = {
                "Tri à bulles": self._bubble_sort_anim,
                "Tri par sélection": self._selection_sort_anim,
                "Tri par insertion": self._insertion_sort_anim,
                "Tri rapide": self._quick_sort_anim,
                "Tri fusion": self._merge_sort_anim,
                "Tri par tas": self._heap_sort_anim,
                "Tri peigne": self._comb_sort_anim
            }
            sort_anim = algo_func.get(selected)
            if sort_anim:
                sort_anim(data, self.draw_step)
    ...
    def draw_step(self, arr, highlights=None):
        ...
        palette = []
        for i in range(len(arr)):
            if i in highlights.get("sorted", []):
                palette.append("#00FF99")  # vert fluo pour éléments triés
            else:
    ...
    def _quick_sort_anim(self, arr, draw_callback):
        ...
        draw_callback(arr, highlights={"sorted": list(range(len(arr)))})
    ...
    def _bubble_sort_anim(self, arr, draw_callback):
        n = len(arr)
        for i in range(n):
    ...
    def _selection_sort_anim(self, arr, draw_callback):
            n = len(arr)
            for i in range(n):
                min_idx = i
    ...
    def _insertion_sort_anim(self, arr, draw_callback):
    ...
    def _heap_sort_anim(self, arr, draw_callback):
            def heapify(arr, n, i):
                largest = i
                l = 2 * i + 1
                r = 2 * i + 2
    ...
    def _comb_sort_anim(self, arr, draw_callback):
            gap = len(arr)
            shrink = 1.3
            sorted = False
            while not sorted:
...
class SortCompetitionFrame(ctk.CTkToplevel):
    ...
    def _save_plot(self):
        ...
        # Removed the undefined draw_callback call as it is not relevant here
        pass