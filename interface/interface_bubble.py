    ...
    def sort(self, arr):
        ...
        for i in range(n):
            for j in range(0, n - i - 1):
                self.draw_step(arr, {"compare": [j, j + 1]})
                if arr[j] > arr[j + 1]:
                    arr[j], arr[j + 1] = arr[j + 1], arr[j]
                    self.draw_step(arr, {"swap": [j, j + 1]})
            # Marquer la fin de chaque passe
            self.draw_step(arr, {"sorted": list(range(n - i, n))})
        self.draw_step(arr, {"sorted": list(range(n))})